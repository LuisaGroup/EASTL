...
namespace eastl
{
	...
	inline OutputIterator move_and_copy_chooser(InputIterator first, InputIterator last, OutputIterator result)
	{
		...
		const bool canBeMemmoved = eastl::is_trivially_copyable<value_type_output>::value &&
								  ...
								  (eastl::is_pointer<InputIterator>::value  || eastl::is_same<IIC, std::contiguous_iterator_tag>::value) &&
								  (eastl::is_pointer<OutputIterator>::value || eastl::is_same<OIC, std::contiguous_iterator_tag>::value);
