...
#define EASTL_TYPE_TRAIT_is_array_cv_convertible_CONFORMANCE 1
		...
		template <typename P1,
		          typename P2,
		          bool = eastl::is_same_v<eastl::remove_cv_t<typename pointer_traits<P1>::element_type>,
		                                  eastl::remove_cv_t<typename pointer_traits<P2>::element_type>>>
		struct is_array_cv_convertible_impl : public eastl::is_convertible<P1, P2>
		{
		}; // Return true if P1 is convertible to P2.

		template <typename P1, typename P2>
		struct is_array_cv_convertible_impl<P1, P2, false> : public eastl::false_type
		{
		}; // P1's underlying type is not the same as P2's, so it can't be converted, even if P2 refers to a subclass of
		   // P1. Parent == Child, but Parent[] != Child[]
		...
		template <typename P1, typename P2>
		struct is_array_cv_convertible<P1, P2, true> : public eastl::false_type
		{
		...
		// but returns false if Derived is the same as Base. So is_derived is true only if Derived is actually a
		// subclass of Base and not Base itself.
		//
...
#define EASTL_TYPE_TRAIT_is_derived_CONFORMANCE 1
		...
		struct is_derived
		    : public eastl::integral_constant<bool,
		                                      eastl::is_base_of<Base, Derived>::value &&
		                                          !eastl::is_same<typename eastl::remove_cv<Base>::type,
		                                                          typename eastl::remove_cv<Derived>::type>::value>
		{
		};
#else
#define EASTL_TYPE_TRAIT_is_derived_CONFORMANCE 0
		...
		template <typename Base,
		          typename Derived> // This returns true if Derived is unrelated to Base. That's a wrong answer, but is
		                            // better for us than returning false for compilers that don't support is_base_of.
		struct is_derived
		    : public eastl::integral_constant<bool,
		                                      !eastl::is_same<typename eastl::remove_cv<Base>::type,
		                                                      typename eastl::remove_cv<Derived>::type>::value>
		...
		struct is_safe_array_conversion
		    : public eastl::integral_constant<
		          bool,
		          eastl::is_convertible<U_pointer, T_pointer>::value && eastl::is_array<U>::value &&
		              (!eastl::is_pointer<U_pointer>::value || !is_pointer<T_pointer>::value ||
		               !Internal::is_derived<T, typename eastl::remove_extent<U>::type>::value)>
		{
		};
	...
	struct default_delete
	{
		...
		default_delete(const default_delete<U>&,
		               typename eastl::enable_if<is_convertible<U*, T*>::value>::type* = 0) EA_NOEXCEPT
		{
		}

		void operator()(T* p) const EA_NOEXCEPT
		{
			static_assert(eastl::internal::is_complete_type_v<T>,
			              "Attempting to call the destructor of an incomplete type");
			p->~T();
	...
	struct default_delete<T[]> // Specialization for arrays.
	{
		...
		template <typename U> // This ctor is enabled if T is equal to or a base of U, and if U is less or equal
		                      // const/volatile-qualified than T.
		default_delete(const default_delete<U[]>&,
		               typename eastl::enable_if<Internal::is_array_cv_convertible<U*, T*>::value>::type* = 0)
		    EA_NOEXCEPT
		{
		}
