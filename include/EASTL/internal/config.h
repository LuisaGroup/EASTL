...
//     printf("EASTL version: %d.%d.%d", EASTL_VERSION_N / 10000 % 100, EASTL_VERSION_N / 100 % 100, EASTL_VERSION_N %
//     100);
//
///////////////////////////////////////////////////////////////////////////////
...
// _HAS_EXCEPTIONS is defined by Dinkumware to 0 or 1 (disabled or enabled).
    ...
    (defined(_MSC_VER) && defined(EA_HAVE_DINKUMWARE_CPP_LIBRARY) && !(defined(_HAS_EXCEPTIONS) && _HAS_EXCEPTIONS))
...
#if defined(EASTL_BITSET_WORD_SIZE) // EASTL_BITSET_WORD_SIZE is deprecated, but we temporarily support the ability for
                                    // the user to specify it. Use EASTL_BITSET_WORD_TYPE_DEFAULT instead.
...
#if ((defined(__GNUC__) && (__GNUC__ >= 3)) || defined(__clang__)) && (!defined(__i386__) || defined(__x86_64__)) && \
    !defined(__ppc__) && !defined(__PPC__) && !defined(__PPC64__)
...
/* Disabled because we don't currently need it.
    ...
    #pragma warning(push, 0)
        #include <yvals.h>
    #pragma warning(pop)
    #if ((defined(_HAS_TR1) && _HAS_TR1) || _MSC_VER >= 1700)  // VS2012 (1700) and later has built-in type traits
support. #define EASTL_STD_TYPE_TRAITS_AVAILABLE 1 #include <type_traits> #else #define EASTL_STD_TYPE_TRAITS_AVAILABLE
0 #endif

#elif defined(EA_COMPILER_CLANG) || (defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION >= 4003) && !defined(__GCCXML__))
&& !defined(EA_COMPILER_NO_STANDARD_CPP_LIBRARY) #include <cstddef> // This will define __GLIBCXX__ if using GNU's
libstdc++ and _LIBCPP_VERSION if using clang's libc++.

    #if defined(EA_COMPILER_CLANG) && !defined(EA_PLATFORM_APPLE) // As of v3.0.0, Apple's clang doesn't support type
traits.
        ...
        #elif defined(__GLIBCXX__) && (__GLIBCXX__ >= 20090124) // It's not clear if this is the oldest version that has
type traits; probably it isn't. #define EASTL_STD_TYPE_TRAITS_AVAILABLE 1

            #if defined(__GXX_EXPERIMENTAL_CXX0X__) // To do: Update this test to include conforming C++11
implementations. #include <type_traits> #else #include <tr1/type_traits> #endif #else #define
EASTL_STD_TYPE_TRAITS_AVAILABLE 0 #endif #endif
...
//         enum : size_type { npos = container_type::npos };    // Microsoft extension which results in significantly
//         smaller debug symbols.
//     #else
...
#if EASTL_NO_RVALUE_REFERENCES // If the compiler doesn't support rvalue references or EASTL is configured to disable
                               // them...
...
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(EA_HAVE_LIBSTDCPP_LIBRARY) && defined(EA_COMPILER_GNUC) && \
...
///////////////////////////////////////////////////////////////////////////////
    ...
    (__EDG_VERSION__ >= 403) // It may in fact be supported by 4.01 or 4.02 but we don't have compilers to test with.
...
#ifndef EASTLAlloc // To consider: Instead of calling through pAllocator, just go directly to operator new, since that's
                   // what allocator does.
#define EASTLAlloc(allocator, n) (allocator).allocate(n);
#endif

#ifndef EASTLAllocFlags // To consider: Instead of calling through pAllocator, just go directly to operator new, since
                        // that's what allocator does.
...
/// This should be fixed in a future release of VS2013
/// http://accentuable4.rssing.com/browser.php?indx=3511740&item=15696
///
