...
namespace eastl
{
	...
	public:
	   ...
	   ~vector();
		...
		template<class... Args>
#if EASTL_ENABLE_VARIADIC_TEMPLATES
		// The is_constructible check is to prevent this from being used with initializer lists, 
		// as the standard specifies that emplace_back cannot be used with initializer lists.
		// We use requires if available, else enable_if.
	...
	template <typename T, typename Allocator>
	void vector<T, Allocator>::resize_uninitialized(size_type n)
	{
		if (n > size_type(internalCapacityPtr() - mpBegin))
		{
	...
	template <typename T, typename Allocator>
	inline void* vector<T, Allocator>::push_back_uninitialized(size_type count)
	{
